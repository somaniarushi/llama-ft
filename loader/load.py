...
class IntelOrcaTokensDataLoader(ParquetDictDataLoader):
    ...
    def row_to_tokens(self, row: dict, get_chosen: bool = True) -> dict:
        ...
        question_tokens: List[int] = self.tokenizer.encode(row['question'], bos=True, eos=False)
        question_mask = [0] * len(question_tokens) # No loss on the questions
        
        chosen_tokens: List[int] = self.tokenizer.encode(row['chosen'], bos=False, eos=True)
        chosen_mask = [1] * len(chosen_tokens)
        
        rejected_tokens: List[int] = self.tokenizer.encode(row['rejected'], bos=False, eos=True)
        ...
        if get_chosen:
            ...
            return IntelOrcaTokens(tokens=torch.tensor(tokens), loss_mask=torch.tensor(mask))
        else:
